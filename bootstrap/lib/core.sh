...
bootstrap_hash() { [[ ${BOOTSTRAP_HASH_TOOL:-} != missing ]] || return 1; if command -v sha256sum >/dev/null 2>&1; then sha256sum | awk '{print "sha256:" $1}'; elif command -v shasum >/dev/null 2>&1; then shasum -a 256 | awk '{print "sha256:" $1}'; else return 1; fi; }
...
bootstrap_fixture_guard() {
  ...
  [[ "$HOME" != "${REAL_HOME:-$HOME}" && -f "${HOME%/*}/.omarchy-bootstrap-fixture" ]] || { bootstrap_error "FIXTURE_GUARD_FAILED"; return "$BOOTSTRAP_INVALID"; }
}
