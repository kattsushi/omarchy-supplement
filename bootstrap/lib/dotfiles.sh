...
dotfiles_status() {
	...
	[[ -L $checkout ]] && {
		printf 'DOTFILES_SYMLINKED_ROOT\n'
		return
	}
	[[ -d $checkout ]] || {
		printf 'DOTFILES_MISSING\n'
		return
	}
	git -C "$checkout" rev-parse --is-inside-work-tree >/dev/null 2>&1 || {
		printf 'DOTFILES_NOT_GIT\n'
		return
	}
	remote=$(git -C "$checkout" remote get-url origin 2>/dev/null || true)
	[[ $remote == https://github.com/kattsushi/dotfiles-v2.git || $remote == git@github.com:kattsushi/dotfiles-v2.git || $remote == ssh://git@github.com/kattsushi/dotfiles-v2.git ]] || {
		printf 'DOTFILES_REMOTE_MISMATCH\n'
		return
	}
	head=$(git -C "$checkout" rev-parse HEAD 2>/dev/null || true)
	[[ $expected =~ ^[0-9a-f]{40}$ && $head == "$expected" ]] || {
		printf 'DOTFILES_REVISION_MISMATCH\n'
		return
	}
	[[ -z $(git -C "$checkout" status --porcelain) ]] || {
		printf 'DOTFILES_DIRTY\n'
		return
	}
...
ownership_status() {
	...
	while IFS=$'\t' read -r _ ref package source target; do
		...
		source_path="$checkout/$package/$source"
		target_path="$HOME/$target"
		[[ -d "$checkout/$package" && -e "$source_path" ]] || {
