#!/usr/bin/env bash
plan_emit() {
  local catalog=$1 os=$2 arch=$3; shift 3
  ...
  engine_digest=$(bootstrap_digest_files "$ROOT/bin/workstation-bootstrap" "$ROOT/bootstrap/lib/core.sh" "$ROOT/bootstrap/lib/catalog.sh" "$ROOT/bootstrap/lib/providers.sh" "$ROOT/bootstrap/lib/dotfiles.sh" "$ROOT/bootstrap/lib/resolve.sh" "$ROOT/bootstrap/lib/plan.sh") || return "$BOOTSTRAP_REFUSAL"
  ...
  while IFS=$'\t' read -r order id kind required provider desired status; do
    ...
    if [[ $kind == dotfiles ]]; then
      if [[ ! $pin_revision =~ ^[0-9a-f]{40}$ ]]; then status=PIN_MISSING
      elif [[ -z $pin_integrity || $pin_integrity == - ]]; then status=INTEGRITY_MISSING
  ...
  (( action_count > 0 )) || { bootstrap_error "NO_ACTIONS"; return "$BOOTSTRAP_INVALID"; }
