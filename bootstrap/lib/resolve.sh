...
resolve_profiles() {
  ...
  while ((${#queue[@]})); do
    ...
    [[ -n $row ]] || { bootstrap_error "PROFILE_UNKNOWN:$name"; return "$BOOTSTRAP_INVALID"; }
    IFS=$'\t' read -r record name row_os architectures implies conflicts <<< "$row"
    [[ $row_os == "$os" ]] && case ",$architectures," in *,"$arch",*) true;; *) false;; esac || { printf 'PROFILE_INCOMPATIBLE\t%s\t%s\n' "$name" "$os"; return "$BOOTSTRAP_REFUSAL"; }
    expanded+=("$name")
  ...
  for selected in "${expanded[@]}"; do
    ...
    [[ " ${expanded[*]} " == *" $conflict "* ]] && { bootstrap_error "PROFILE_CONFLICT:$selected:$conflict"; return "$BOOTSTRAP_REFUSAL"; }
    done
  done
  printf '%s\n' "${expanded[@]}" | LC_ALL=C sort -u
}
resolve_actions() {
  local catalog=$1 os=$2; shift 2; local profiles=" $* " row profile provider id kind required order desired status
  while IFS=$'\t' read -r _ profile order id kind required provider desired; do
    [[ " $profiles " == *" $profile "* ]] || continue
        provider_allowed "$os" "$provider" || continue
