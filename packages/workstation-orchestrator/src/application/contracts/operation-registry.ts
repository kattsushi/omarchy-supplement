...
export const makeReadOnlyOperationHandlers = (assessment: AssessmentService, planning: PlanningService, backups: BackupService, observations: ObservationService): OperationHandlers => ({
  ...
  plan_package_install: (request) => request.operation !== "plan_package_install" ? unavailable(request) : Effect.map(planning.plan({
    ...
    version: "PublicResultV2", operation: "plan_package_install", status: "refused", correlationId: request.requestId, payload: { kind: "unavailable", operation: "plan_package_install", reason: "source-unavailable" }, blockers: [{ code: "operation-refused" }], evidence: (value.evidence ?? []).map((record: { readonly evidenceId: string; readonly strength: string; readonly summaryCode: string }) => ({ evidenceId: record.evidenceId, strength: record.strength, summaryCode: record.summaryCode })), nextActions: value.nextActions.map((action: any) => action.reasonCode),
  } : {
