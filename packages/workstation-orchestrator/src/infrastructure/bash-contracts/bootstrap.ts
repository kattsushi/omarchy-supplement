...
export type Brf = { readonly profiles: readonly string[]; readonly complete: boolean; readonly actions: readonly { readonly id: string; readonly state: "noop" | "execute" | "blocked" }[] };
...
export const parseBrf = (text: string): Result.Result<Brf, InvalidContract | Refused> => {
  ...
  return Result.succeed({ profiles: profiles.filter((value): value is string => value !== undefined), actions, complete: true });
};
