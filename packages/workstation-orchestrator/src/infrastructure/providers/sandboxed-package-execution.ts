...
import { HomebrewExecutionPort, OmarchyExecutionPort, PackageExecutionUnavailable, type HomebrewExecutionInput, type OmarchyExecutionInput, type ProviderExecutionReport } from "../../application/ports/package-execution.js";
...
const run = (config: ProviderSandboxConfig, expectedExecutable: "omarchy" | "brew", argv: readonly string[]) => Effect.tryPromise({
  try: async (): Promise<ProviderExecutionReport> => {
    ...
    const subprocess = spawn(config.executable, argv.slice(1), { cwd: config.cwd, env: { ...config.env }, stdio: ["ignore", "pipe", "pipe"], shell: false });
    return await new Promise<ProviderExecutionReport>((resolve, reject) => {
      ...
      const timer = setTimeout(() => { subprocess.kill(); resolve({ kind: "timed-out", evidenceIds: [] }); }, config.timeoutMs);
      subprocess.on("close", (code) => {
        clearTimeout(timer);
        resolve(oversized ? { kind: "malformed", evidenceIds: [] } : decodeReport(stdout.trim(), code ?? 1));
      });
    });
  },
