...
const sandbox = async (mode: "success" | "partial" | "failure" | "timeout" | "malformed" | "unverifiable") => {
  ...
  for (const name of ["omarchy", "brew"]) {
    await writeFile(`${paths.bin}/${name}`, fixture);
    await chmod(`${paths.bin}/${name}`, 0o700);
  }
  return paths;
};

const binding = (provider: "omarchy" | "homebrew"): PlanBindingInput => ({
  operation: "package-acquisition", logicalRequestIds: ["program:neovim"], platformObservationDigest: "observation:sandbox",
  profilePolicyDigest: "policy:sandbox", provider, providerRole: provider === "omarchy" ? "primary" : "fallback",
  providerPolicy: { id: `policy:${provider}`, version: "1" }, capabilityId: provider === "omarchy" ? "omarchy-pkg-add" : "homebrew-formula",
  mappingIds: [`mapping:${provider}-neovim`], packageStateDigests: ["state:missing"], verificationPolicyId: "verification:sandbox",
  riskCodes: ["network"], fallbackOptIn: provider === "homebrew",
});
...
const config = (paths: Awaited<ReturnType<typeof sandbox>>, provider: "omarchy" | "homebrew") => ({
  ...
  executable: `${paths.bin}/${provider === "omarchy" ? "omarchy" : "brew"}`,
  cwd: paths.cwd,
  ...
  env: {
    ...
    PATH: paths.bin,
    CAPTURE_FILE: paths.capture,
  },
});

describe("sandboxed provider execution adapters", () => {
  it.each([
    ["omarchy", ["omarchy", "pkg", "add", "neovim"]],
    ["homebrew", ["brew", "install", "neovim"]],
  ] as const)("executes only the fixture %s argv in an isolated environment", async (provider, argv) => {
    ...
    expect(JSON.parse(await readFile(paths.capture, "utf8"))).toEqual({
      executable: provider === "omarchy" ? "omarchy" : "brew",
      argv: argv.slice(1), cwd: paths.cwd, home: paths.home, path: paths.bin,
    });
  ...
  ] as const)("returns a bounded %s outcome without retry or rollback", async (mode, expected) => {
  ...
  it("refuses executables and working directories outside the sandbox", async () => {
    ...
    const plan = await packagePlan("omarchy");
    const input: OmarchyExecutionInput = { plan: plan as OmarchyPackagePlan, command: { provider: "omarchy", policyId: plan.plan.binding.providerPolicy.id, argv: ["omarchy", "pkg", "add", "neovim"] } };
    const outside = sandboxedOmarchyExecutionLayer({ ...config(paths, "omarchy"), executable: "/usr/bin/true" });
    const result = await Effect.runPromise(Effect.flip(Effect.flatMap(OmarchyExecutionPort, (port) => port.execute(input)).pipe(Effect.provide(outside))));
