...
describe("package execution capability boundaries", () => {
  it("keeps executors out of readOnlyLayer, CLI, TUI, and public operation contracts", async () => {
    ...
    for (const content of contents) expect(content).not.toMatch(/OmarchyExecutionPort|HomebrewExecutionPort|ExecutePackagePlan|sandboxed-package-execution|composition\/mutation/);
  });
  ...
  it("contains no shell-string execution, eval, retry, resume, rollback, sudo, or Homebrew installation path", async () => {
    const executionFiles = ["application/ports/package-execution.ts", "application/services/package-execution.ts", "infrastructure/providers/sandboxed-package-execution.ts", "composition/mutation.ts"];
