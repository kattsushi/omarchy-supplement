...
classify() {
  ...
  case "$1" in
    README.md|polybar/.config/polybar/config.ini|starship/.config/starship.toml|zshrc/.config/zsh/common.zsh|zshrc/.config/zsh/linux.zsh) security=reviewed-reference ;;
    ...
    systems/darwin/*) package=systems/darwin; class=macos; disposition=exclude; reason=machine-specific-darwin; security=blocked ;;
    ...
    zshrc/.config/zsh/darwin.zsh) package=zshrc; class=macos; security=reviewed-reference ;;
...
validate() {
  ...
  while IFS=$'\t' read -r kind path type mode size digest package class disposition reason security extra; do
    ...
    [[ "$disposition" =~ ^(include|exclude)$ && "$security" =~ ^(clear|reviewed-reference|blocked)$ ]] || return 2
    [ "$security" != blocked ] || [ "$disposition" = exclude ] || return 2
